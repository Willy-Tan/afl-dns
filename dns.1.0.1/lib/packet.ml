...
(*Modified by Willy to support Crowbar uses*)
(*IP adresses*)

(*Maybe get a better generator than Crowbar.int *)
(*I have to explicitly define a _to_crowbar function for future [@@deriving crowbar]*)
type ipv4 = Ipaddr.V4.t;;
let ipv4_to_crowbar = Crowbar.map [Crowbar.int;Crowbar.int;Crowbar.int;Crowbar.int] Ipaddr.V4.make;;


(*I have to explicitly define a _to_crowbar function for future [@@deriving crowbar]*)
type ipv6 = Ipaddr.V6.t;;
let ipv6_to_crowbar = Crowbar.map [Crowbar.int;Crowbar.int;Crowbar.int;Crowbar.int;Crowbar.int;Crowbar.int;Crowbar.int;Crowbar.int] Ipaddr.V6.make;;
...
let rr_type_to_crowbar =
  Crowbar.choose [
    ...
    Crowbar.const RR_MD;         
    Crowbar.const RR_MF;         
    Crowbar.const RR_CNAME;      
    Crowbar.const RR_SOA;        
    Crowbar.const RR_MB;         
    Crowbar.const RR_MG;         
    Crowbar.const RR_MR;         
    Crowbar.const RR_NULL;       
    Crowbar.const RR_WKS;        
    Crowbar.const RR_PTR;        
    Crowbar.const RR_HINFO;      
    Crowbar.const RR_MINFO;      
    Crowbar.const RR_MX;         
    Crowbar.const RR_TXT;        
    Crowbar.const RR_RP;         
    Crowbar.const RR_AFSDB;      
    Crowbar.const RR_X25;        
    Crowbar.const RR_ISDN;       
    Crowbar.const RR_RT;         
    Crowbar.const RR_NSAP;       
    Crowbar.const RR_NSAPPTR;    
    Crowbar.const RR_SIG;        
    Crowbar.const RR_KEY;        
    Crowbar.const RR_PX;         
    Crowbar.const RR_GPOS;       
    Crowbar.const RR_AAAA;       
    Crowbar.const RR_LOC;        
    Crowbar.const RR_NXT;        
    Crowbar.const RR_EID;        
    Crowbar.const RR_NIMLOC;     
    Crowbar.const RR_SRV;        
    Crowbar.const RR_ATMA;       
    Crowbar.const RR_NAPTR;      
    Crowbar.const RR_KM;         
    Crowbar.const RR_CERT;       
    Crowbar.const RR_A6;         
    Crowbar.const RR_DNAME;      
    Crowbar.const RR_SINK;       
    Crowbar.const RR_OPT;        
    Crowbar.const RR_APL;        
    Crowbar.const RR_DS;         
    Crowbar.const RR_SSHFP;      
    Crowbar.const RR_IPSECKEY;   
    Crowbar.const RR_RRSIG;      
    Crowbar.const RR_NSEC;      
    Crowbar.const RR_DNSKEY;     
    Crowbar.const RR_NSEC3;      
    Crowbar.const RR_NSEC3PARAM;
    Crowbar.const RR_SPF;        
    Crowbar.const RR_UINFO;      
    Crowbar.const RR_UID;        
    Crowbar.const RR_GID;        
    Crowbar.const RR_UNSPEC;
  ];;
 
let digest_alg_to_crowbar =
  Crowbar.choose [
    Crowbar.const (SHA1 : digest_alg);
    Crowbar.const (SHA256 : digest_alg);
  ];;

let dnssec_alg_to_crowbar =
  Crowbar.choose [
    Crowbar.const RSAMD5;
    Crowbar.const DH;
    Crowbar.const DSA;
    Crowbar.const ECC;
    Crowbar.const RSASHA1;
    Crowbar.const RSANSEC3;
    Crowbar.const RSASHA256;
    Crowbar.const RSASHA512;
    Crowbar.const INDIRECT;
    Crowbar.const PRIVATEDNS;
    Crowbar.const PRIVATEOID;
  ];;

let gateway_tc_to_crowbar = 
  Crowbar.choose [
    Crowbar.const (NONE : gateway_tc);
    Crowbar.const (IPv4 : gateway_tc);
    Crowbar.const (IPv6 : gateway_tc);
    Crowbar.const (NAME : gateway_tc);
  ];;


let ipseckey_alg_to_crowbar =
  Crowbar.choose [
    Crowbar.const (DSA : ipseckey_alg);
    Crowbar.const (RSA : ipseckey_alg);
  ];;


let hash_alg_to_crowbar = Crowbar.const (SHA1 : hash_alg);;
 

let pubkey_alg_to_crowbar =
  Crowbar.choose [
    Crowbar.const (RESERVED : pubkey_alg);
    Crowbar.const (RSA : pubkey_alg);
    Crowbar.const (DSS : pubkey_alg)
  ];;


let fp_type_to_crowbar = Crowbar.const (SHA1 : fp_type);;

let gateway_to_crowbar =
  Crowbar.choose [
    Crowbar.map [ipv4_to_crowbar] (fun ipv4 -> (IPv4 ipv4 : gateway) ) ;
    Crowbar.map [ipv6_to_crowbar] (fun ipv6 -> (IPv6 ipv6 : gateway) ) ;
    Crowbar.map [Name.to_crowbar] (fun name -> (NAME name : gateway) ) ;
  ];;
...
(*
   ...
   holds glue records.
*)
type type_bit_map = Cstruct.byte * Cstruct.byte * Cstruct.t [@@deriving crowbar]
let type_bit_map_to_string (tbm:type_bit_map) : string =
  "TYPE_BIT_MAP"

type type_bit_maps = type_bit_map list [@@deriving crowbar]
...
let rr_class_to_crowbar =
  Crowbar.choose [
    Crowbar.const RR_IN;
    Crowbar.const RR_CS;
    Crowbar.const RR_CH;
    Crowbar.const RR_HS;
    Crowbar.const RR_ANY;
  ];;
...
let q_class_to_crowbar =
  Crowbar.choose [
    Crowbar.const Q_IN;
    Crowbar.const Q_CS;
    Crowbar.const Q_CH;
    Crowbar.const Q_NONE;
    Crowbar.const Q_ANY_CLS;
  ];;
...
type q_unicast = Q_Normal | Q_mDNS_Unicast [@@deriving crowbar];;
...
let opcode_to_crowbar =
  Crowbar.choose [
    Crowbar.const Standard;
    Crowbar.const Inverse;
    Crowbar.const Status;
    Crowbar.const Reserved;
    Crowbar.const Notify;
    Crowbar.const Update;
  ];;
...
let rcode_to_crowbar =
  Crowbar.choose [
    Crowbar.const NoError;
    Crowbar.const FormErr;
    Crowbar.const ServFail;
    Crowbar.const NXDomain;
    Crowbar.const NotImp;
    Crowbar.const Refused;
    Crowbar.const YXDomain;
    Crowbar.const YXRRSet;
    Crowbar.const NXRRSet;
    Crowbar.const NotAuth;
    Crowbar.const NotZone;
    Crowbar.const BadVers;
    Crowbar.const BadKey;
    Crowbar.const BadTime;
    Crowbar.const BadMode;
    Crowbar.const BadName;
    Crowbar.const BadAlg;
  ];;
...
(*Small modification by willy, typing question list*)
type questions = question list;;

type t = {
  id          : int;
  detail      : detail;
  questions   : questions; (* Cstruct.iter; *)
...
let list_to_questions (q : question list) = (q : questions);; 

let questions_to_crowbar = Crowbar.map [Crowbar.list1 question_to_crowbar] list_to_questions;;

let rec print_question (questions : questions) = match questions with
  |[] -> ()
  |t::q -> (Printf.printf "%s\n %!" @@ question_to_string t; print_question q);;
...
let to_crowbar =
  ...
  let authorities = Crowbar.list1 rr_to_crowbar in
  let additionals = Crowbar.list1 rr_to_crowbar in
  Crowbar.map [id;detail;questions;answers;authorities;additionals] @@
  (fun i de qu an au ad ->
     {
       id = i ;
       detail = de;
       questions = qu;
