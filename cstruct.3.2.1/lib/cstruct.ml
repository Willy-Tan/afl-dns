...
type uint8 = int [@@deriving crowbar];;
type uint16 = int [@@deriving crowbar];;
type uint32 = int32 [@@deriving crowbar];;
type uint64 = int64 [@@deriving crowbar];;
...
(*Subject to changes : the result may be invalid*)

let byte_to_crowbar = Crowbar.map [Crowbar.range 255] byte;;

let to_crowbar = Crowbar.map [Crowbar.bytes] (fun a ->
    try
      of_string a
    with
    _ -> Crowbar.bad_test ());;
